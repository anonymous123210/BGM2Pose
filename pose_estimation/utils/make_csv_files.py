...
def get_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="music type, chosen from [Jazz, Ambient, Techno]",
    )
...
def split_posedata_timeseries_name(sound_data, name:str, music=None) -> Tuple[DataFrame]:
    if music!=None:
        columns = ["sound_path", "testee", "sound_length",'music_type'] + get_joints()
    else:
        columns = ["sound_path", "testee", "sound_length"] + get_joints()
    csv_data = {
        "train": {name: [] for name in columns},
    ...
    if music!=None:
        data = data[data.music_type.isin(music)].reset_index(drop=True)
    data = data.set_index("sound_path").reset_index()
...
def split_posedata_kfold_name(sound_data, name, music=None, except_ = []) -> Tuple[DataFrame]:
    if music!=None:
        columns = ["sound_path", "testee", "spec_duration", "music_type", "label", "joint_path"] #+ get_joints()
    else:
        columns = ["sound_path", "testee", "spec_duration", "label", "joint_path"] #+ get_joints()

    data = sound_data[columns]
    data = data.set_index("sound_path")
    if music!=None:
        data = data[data.music_type.isin(music)]
    if except_ != None:
        data = data[~data.testee.isin(except_)]
    train_df = data[
        (data.testee != name)
    ].reset_index()
    val_df = data[
...
def main() -> None:
    ...
    if args.dataset_name not in DATASET_CSVS:
        ...
        if ('timeseries' not in args.dataset_name) & ('kfold' not in args.dataset_name):
    ...
    elif args.dataset_name == 'timeseries_same_music':
        train_df, val_df, test_df = split_posedata_timeseries_name(sound_data, args.subject_name, music=args.music)
    ...
    elif args.dataset_name == 'kfold_same_music':
        train_df, val_df, test_df = split_posedata_kfold_name(sound_data, args.subject_name, music=args.music)
    
    elif args.dataset_name == 'kfold_same_music_wo_subject_3':
        train_df, val_df, test_df = split_posedata_kfold_name(sound_data, args.subject_name, music=args.music, except_ = ['subject_3'])
        
    elif args.dataset_name == 'kfold_same_music_wo_subject_1_3':
        train_df, val_df, test_df = split_posedata_kfold_name(sound_data, args.subject_name, music=args.music, except_ = ['subject_1','subject_3'])
    ...
    elif "wo_mocap" in args.dataset_name:
        train_df, val_df, test_df = sound_data[sound_data.testee==args.subject_name].copy(), sound_data[sound_data.testee==args.subject_name].copy(), sound_data[sound_data.testee==args.subject_name].copy()
    ...
    # kfoldの場合は名前もつける
    # if (args.dataset_name == 'pose_regression_timeseries') | (args.dataset_name == 'pose_regression_kfold'):
    if ('timeseries' in args.dataset_name) | ('kfold' in args.dataset_name):
        val_dir = args.dataset_name + '_' + args.subject_name
        if ("music" in args.dataset_name):
            if len(args.music) == 4:
                val_dir+='_all'
            else:
                val_dir+= '_' + '_'.join(args.music)
        path = os.path.join("./csv", val_dir, "train.csv")
        train_df, val_df, test_df = update_df(train_df, val_df, test_df, path, preprocess=args.preprocess)
        print('train size:', train_df.shape)
        print('val size:', val_df.shape)
        print('test size:', test_df.shape)
        os.makedirs(
            os.path.join("./csv", val_dir),
            exist_ok=True,
        )
        train_df.to_csv(
            os.path.join("./csv", val_dir,"train.csv"),
            index=None,
        )
        val_df.to_csv(
            os.path.join("./csv", val_dir,"val.csv"),
            index=None,
        )
        test_df.to_csv(
            os.path.join("./csv", val_dir,"test.csv"),
            index=None,
        )
    else:
        # 保存ディレクトリがなければ，作成
