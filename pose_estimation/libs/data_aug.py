...
def filt_aug(features, db_range=[-6, 6], n_band=[3, 6], min_bw=6, filter_type="linear"):
    ...
    if n_freq_band > 1:
        ...
        if filter_type == "step":
            band_factors = torch.rand((n_freq_band)).to(features) * (db_range[1] - db_range[0]) + db_range[0]
            band_factors = 10 ** (band_factors / 20)

            freq_filt = torch.ones((n_freq_bin, 1)).to(features)
            for i in range(n_freq_band):
                freq_filt[:, band_bndry_freqs[i]:band_bndry_freqs[i + 1], :] = band_factors[:, i].unsqueeze(-1).unsqueeze(-1)

        elif filter_type == "linear":
            band_factors = torch.rand((n_freq_band + 1)).to(features) * (db_range[1] - db_range[0]) + db_range[0]
            freq_filt = torch.ones((n_freq_bin, 1)).to(features)
            for i in range(n_freq_band):
...
def frame_shift(features, label=None, net_pooling=None):
    if label is not None:
        ...
        shift = int(random.gauss(0, 5))
        return torch.roll(features, shift, dims=-1)
...
def freq_mask(features, mask_ratio=16):
    channel, _, n_freq_bin = features.shape
    max_mask = int(n_freq_bin/mask_ratio)
    f_widths = torch.randint(low=1, high=max_mask, size=(1,))   # [low, high)
    f_width = f_widths[0]
