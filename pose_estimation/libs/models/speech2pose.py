...
class Speech2pose(nn.Module):
    def __init__(self, in_cha = 4, out_cha = 4, seq_len=12, dropout = 0.0):
        super().__init__()
        self.in_cha = in_cha
        self.cha_1 = 64
        self.down_k = 4
    ...
    def forward(self, mel, music_mask): #mel -> (bs, in_cha = 4, time, freq)
        seq_len = mel.shape[2]
        in_cha = mel.shape[1]
        if (in_cha == 9) & (self.in_cha==7):
            mel = torch.concat([mel[:,:4,:,:], mel[:,-3:,:,:]], dim = 1)
        elif (in_cha == 9) & (self.in_cha==1):
            mel = mel[:,:1,:,:]
        elif (in_cha == 9) & (self.in_cha==6):
            mel = mel[:,:6,:,:]
        ...
        if seq_len > 6:
            ...
            if seq_len > 12:
                out_8 = self.eight_block(out_7)
                if seq_len>24:
                    out_9 = self.ninth_block(out_8)
                    out_8 = out_8 + self.upsampling(out_9)
                out_7 = out_7+self.upsampling(out_8)
