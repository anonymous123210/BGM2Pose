...
class Speech2pose_P(nn.Module):
    def __init__(self, in_cha = 4, out_cha = 4):
        ...
        self.seventh_block = get_downsampling_block1_1d(self.cha_4, self.cha_4, k = 2, s = self.s) #[bs, self.sha_4=256, 3]
        ...
        self.mean_bool = nn.AdaptiveAvgPool1d(output_size=1)
    ...
    def forward(self, mel): #mel -> (bs, in_cha = 4, time, freq)
        ...
        # Unet (1D CNN)
        out_5_1 = self.fifth_block(out_4) #[256, 256, 12]
        out_6 = self.sixth_block(out_5_1) #[256, 256, 6]
        out_7 = self.seventh_block(out_6) #[256, 256, 3]
        hidden_out = self.mean_bool(out_7)
        out_6 = out_6 + self.upsampling(out_7) #[256, 256, 6]
        out_5 = out_5_1 + self.upsampling(out_6) #[256, 256, 12]
...
class Speech2Pose_D(nn.Module):
    def __init__(self, in_cha=256, out_cha=5):
        super().__init__()
        self.cha_1 = 256
        self.fc1 = nn.Linear(self.cha_1, out_cha)
    
    def forward(self, x):# x[bs, 256, 12]
        out = nn.Flatten()(x) #[bs, 256 * 3]
        out = self.fc1(out) #[bs, seq_len, 8]
        return out