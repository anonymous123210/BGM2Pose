...
class TrainLogger(object):
    ...
    def update(
        ...
        self.df = self.df.reset_index(drop=True) #self.df.append(tmp, ignore_index=True)
        self._save_log()
        ...
        logger.info(
            f"epoch: {epoch}\tepoch time[sec]: {train_time + val_time}\tlr: {lr}\t"
            f"train loss: {train_loss:.4f}\ttrain d loss:{train_d_loss:.4f}\ttrain d acc:{train_testee_acc:.4f}\t"
            f"val loss: {val_loss:.4f}\tval d loss: {val_loss:.4f}\tval d acc:{val_testee_acc:.4f}\t"
